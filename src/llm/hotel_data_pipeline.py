...
# =============================================
# function: import original generator (updated)
# =============================================
        ...
        rec = {
            ...
            "metadata": {
                **example["metadata"],
                ...
                # winner Hotel snapshot for tie construction
...
def construct_tie_pair(
    ...
    elif spurious_strategy == "standard_monotonic_mixture":
        # per-tie mixture between the decorrelated rule and the standard
        # monotonic (correlated) rule. With probability spurious_strength the
        # tie is decorrelated; with probability 1 - strength the spurious
        # feature follows utility (standard_monotonic_noisy), reinforcing the
        # shortcut. strength=1 recovers fully decorrelated; strength=0 recovers
        # fully monotonic (the failure-mode ablation). The noisy monotonic
        # variant is used so the 1 - strength fraction has nonzero spurious
        # contrast rather than a degenerate Delta_phi_s = 0.
        if random.random() < spurious_strength:
            effective_strategy = "decorrelated_spurious"
        else:
            effective_strategy = "standard_monotonic_noisy"
    else:
    ...
    if spurious_strategy == "decorrelated_mixture":
        ...
        sp_tracking["mixture_strength"] = spurious_strength
        sp_tracking["realized_strategy"] = effective_strategy
        sp_tracking["requested_strategy"] = "standard_monotonic_mixture"
    return hotel_a, hotel_b, util_a, util_b, sp_tracking

# ==========================
# function: assign tie label
# ==========================
...
def render_tie_record(
    ...
    """
    render the tie pair in the candidate-pool.
    """
...
def build_argparser():
    ...
    p.add_argument("--spurious-strategy", nargs="+",
                   ...
                   help="Spurious-axis strategies (train mode only). "
                        ...
                        "standard_monotonic_mixture interpolates between "
                        "standard_monotonic_noisy (--spurious-strength=0) and "
                        "decorrelated_spurious (--spurious-strength=1).")
    p.add_argument("--spurious-strength", type=float, default=0.5,
                   help="Mixture weight for the *_mixture strategies. "
                        "For both decorrelated_mixture and "
                        "standard_monotonic_mixture, this is the per-tie "
                        "probability of applying the decorrelated rule; "
                        "1 - strength applies the other endpoint "
                        "(random_uniform / standard_monotonic_noisy "
                        "respectively). 0.0 = fully the other endpoint, "
                        "1.0 = fully decorrelated_spurious. Default 0.5. "
                        "Across the pool the realized distribution "
