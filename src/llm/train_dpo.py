...
usage:
Strict-only baseline (tie training, alpha = 1.0, local JSONL):

    python train_dpo.py \\
        --train-dataset hotel_data/seed0/aug_a1.0.jsonl \\
        --experiment-name tie_training_a1.0_seed0 \\
        --output-base-dir ./dpo_experiments/ \\
        --num-epochs 1 \\
        --seed 0

Tie training, alpha-mixed augmented set (e.g. alpha = 0.5):

    python train_dpo.py \\
        --train-dataset hotel_data/seed0/aug_a0.5.jsonl \\
        --experiment-name tie_training_a0.5_seed0 \\
        --output-base-dir ./dpo_experiments/ \\
        --num-epochs 1 \\
        --seed 0

NOTE: for the alpha sweep, do NOT pass --train-samples. It truncates after the
shuffle, which throws away ties and distorts the strict/tie ratio (alpha). Leave
it unset so the full augmented file is used and the realized alpha is exact.
...
def main():
    ...
    dataset = dataset.train_test_split(test_size=args.val_fraction, seed=args.seed)
